...
func ValidarDatosMKDIR(context []string, particion Structs.Particion, pth string, w http.ResponseWriter) {
	path := ""
	p := false
	...
	if path == "" {
		...
		MandarError("MKDIR", "Se necesitan parametros obligatorio para crear un directorio.", w)
		return
	}
	tmp := GetPath(path)
	mkdir(tmp, p, particion, pth, w)
}

func mkdir(path []string, p bool, particion Structs.Particion, pth string, w http.ResponseWriter) {
	copia := path
	...
	if err != nil {
		...
		MandarError("MKDIR", "No se ha encontrado el disco.", w)
		return
	}
	...
	if err_ != nil {
		Error("MKDIR", "Error al leer el archivo")
		MandarError("MKDIR", "Error al leer el archivo", w)
		return
	}
	...
	if len(path) == 0 {
		...
		MandarError("MKDIR", "No se ha brindado una path válida", w)
		return
	}
	var past int64
	var bi int64
	...
	for v := 0; v < len(path)-1; v++ {
					...
					for j := 0; j < 4; j++ {
						...
						if Comparar(nombreCarpeta, path[v]) {
							...
							if inode.I_uid != int64(Logged.Uid) {
								...
								MandarError("MKDIR", "No tiene permisos para crear carpetas en este directorio.", w)
								return
							}
		...
		if !fnd {
			if p {
				...
				MandarError("MKDIR", "No se pudo crear el directorio: "+direccion+", no existen directorios.", w)
				return
			}
		}
	}
				...
				for j := 0; j < 4; j++ {
					...
					if folder.B_content[j].B_inodo == -1 {
						...
						if bi == -1 {
							...
							MandarError("MKDIR", "No se ha podido crear el directorio", w)
							return
						}
	...
	MandarMensaje("MKDIR", "Se ha creado el directorio "+ruta, w)
	file.Close()
}
